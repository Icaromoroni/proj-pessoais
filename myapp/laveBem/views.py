...
from .serializers import ServicoSerializer, SolicitacaoSerializer
...
class SolicitacaoListCreate(APIView):
    """
    List all cliente, or create a new cliente.
    """
    def get(self, request, format=None):
        cliente = Solicitacao.objects.all()
        serializer = SolicitacaoSerializer(cliente, many=True)
        return Response(serializer.data)
...
class SolicitacaoDetailUpdate(APIView):
    """
    Retrieve, update or delete a snippet instance.
    """
    def get_object(self, pk):
        try:
            return Solicitacao.objects.get(pk=pk)
        except Solicitacao.DoesNotExist:
            raise Http404

    def get(self, request, pk, format=None):
        cliente = self.get_object(pk)
        serializer = SolicitacaoSerializer(cliente)
        return Response(serializer.data)

    def put(self, request, pk, format=None):
        cliente = self.get_object(pk)
        serializer = SolicitacaoSerializer(cliente, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
